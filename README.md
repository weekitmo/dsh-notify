...
dsh plugin --profile web add \
  git+https://github.com/weekitmo/dsh-notify.git#v0.1.3
```
...
```sh
dsh plugin --profile web add \
  https://github.com/weekitmo/dsh-notify/releases/download/v0.1.3/dsh-notify-0.1.3.tgz
```

Release 同时发布 `SHA256SUMS`。安全或可复现安装建议先固定版本、下载并校验资产，再执行：

```sh
mkdir dsh-notify-v0.1.3 && cd dsh-notify-v0.1.3
curl -fsSLO https://github.com/weekitmo/dsh-notify/releases/download/v0.1.3/install.sh
curl -fsSLO https://github.com/weekitmo/dsh-notify/releases/download/v0.1.3/dsh-notify-0.1.3.tgz
curl -fsSLO https://github.com/weekitmo/dsh-notify/releases/download/v0.1.3/SHA256SUMS
sha256sum -c SHA256SUMS        # Linux
# shasum -a 256 -c SHA256SUMS # macOS
dsh plugin --profile web add "$PWD/dsh-notify-0.1.3.tgz"
```

### 从源码安装

```sh
git clone --branch v0.1.3 --depth 1 https://github.com/weekitmo/dsh-notify.git
cd dsh-notify
corepack enable
